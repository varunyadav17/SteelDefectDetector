...
var connection = mysql.createConnection({
  ...
  user: 'root',
  password: 'root',
  database: 'skincancer_userprofiles',
  insecureAuth: false
});
...
exports.fileprint = function (req, res) {
  ...
  jsonfile.readFile(filepath, function (err, obj) {
    ...
    connection.query('SELECT * FROM userprofiles WHERE userid = ?', [userid], function (error, results, fields) {
            if (error) {
              ...
              if (results.length > 0) {
                ...
                connection.query('UPDATE userprofiles SET printCount = ? WHERE userid = ?', [printCount, userid], function (error, results, fields) {
                  if (error) {
